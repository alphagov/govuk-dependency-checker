...
class DependabotPrometheusMetrics
  ...
  def get_repo_prs(repo)
    ...
    loop do
      puts "#{repo} page: #{page}"
      issues = client.list_issues("alphagov/#{repo}", { state: "all", labels: "dependencies", since: (Date.today - 1).to_s, page: page, per_page: 100 })
  ...
  def fetch_security_alerts(repo)
    client.get("https://api.github.com/repos/alphagov/#{repo}/dependabot/alerts", accept: "application/vnd.github+json", state: "open")
  ...
  def fetch_checks_status(repo, commit_ref)
    check_runs = client.check_runs_for_ref("alphagov/#{repo}", commit_ref)
  ...
  def failing_checks?(repo, pr_number)
    pr_data = client.pull_request("alphagov/#{repo}", pr_number)
  ...
  def update_prometheus_metrics(prometheus_metrics)
    prometheus_metrics[:security_alerts_per_repo].values.each { |metric| metric.set(0) }
    prometheus_metrics[:security_alerts_per_dependency].values.each { |metric| metric.set(0) }
